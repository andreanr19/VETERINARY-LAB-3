...
public class Client {
	...
	/**
	 * Description The method allows to add a object Pet to the arraylist between client and pet.
	 * pre: the pet was created before.
	 * post: the pet is added.
	 * @param newPet
	 */
	...
	/**
	 * Description The method allows to obtain the name of the client. 
	...
	/**
	 * Description The method allows to set the name of the client. 
	 * pre:Existence of an attribute called name
	 * 
	 * @param name
	 */
	...
	/**
	 * Description The method allows to obtain the id of the client. 
	...
	/**
	 * Description The method allows to set the id of the client. 
	 * pre:Existence of an attribute called id
	 * 
	 * @param id
	 */
	...
	/**
	 * Description The method allows to set the address of the client. 
	 * pre:Existence of an attribute called address
	 * 
	 * @param address
	 */
	...
	/**
	 * Description The method allows to set the phone of the client. 
	 * pre:Existence of an attribute called phone
	 * 
	 * @param phone
	 */
	...
	/**
	 * Description This method allows to show the pets of a client
	 * pre: The where created before
	 * post: the pets are shown
	...
	/**
	 * Description The method allows to add a new service to a pet
	 * pre: The pet and the client is created before
	 * post: The service is added to the pet
	 * @param nameOwner
	 * @param namePet
	 * @param type
	 */
	public void addNewServiceToAPet(String nameOwner, String namePet, String type) {
		for (int i = 0; i < petClient.size(); i++) {
			if (petClient.get(i).getName().equals(namePet) && petClient.get(i).getOwner().getName().equals(nameOwner)) {

				petClient.get(i).addServiceToAPet(type);
			}
		}
	}

	/**
	 * Description The method allows to see the actual service a pet has
	 * pre: The pet must have a service different of null
	 * post: the services of the pets are shown
	 * @return a message that says the service of the pet, and in case the pet has no service, the message will say it.
	 */
			...
			if (petClient.get(i).getService() != null) {
				if (!petClient.get(i).getService().getType().equals("ninguno")) {
					msj += "The pet " + petClient.get(i).getName() + " currently has the service of "
							+ petClient.get(i).getService().getType() + "\n";
